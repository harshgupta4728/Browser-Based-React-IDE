...
export default function App(){
  ...
  const theme = window.matchMedia && window.matchMedia('(prefers-color-scheme: light)').matches ? githubLight : githubDark
  const onNewFile = () => {
              ...
              <SandpackProvider files={spFiles} customSetup={{ dependencies: { react: 'latest', 'react-dom': 'latest' } }} theme={theme} options={{ activeFile: activePath }}>
                <SandpackLayout style={{height:'100%'}}>
                  <div className="editor">
                    <SandpackCodeEditor showTabs closableTabs showLineNumbers wrapContent onChange={(c)=> onCodeUpdate(activePath, c||'')} />
                  </div>
                </SandpackLayout>
              </SandpackProvider>
            </div>
          ...
          <Panel defaultSize={40} minSize={25}>
            <div className="right">
              <SandpackProvider files={spFiles} customSetup={{ dependencies: { react: 'latest', 'react-dom': 'latest' } }} theme={theme} options={{ activeFile: activePath, recompileMode: 'delayed', recompileDelay: 300 }}>
                ...
                {consoleOpen && <div className="console"><SandpackConsole showHeader showSyntaxErrors /></div>}
              </SandpackProvider>
            </div>
          </Panel>
        </PanelGroup>
